...
from agent.rag_chain import run_hybrid_agent
...
class HybridQuestionRequest(BaseModel):
    question:         str
    show_cypher:      Optional[bool] = False
    show_doc_sources: Optional[bool] = False
...
class AnomalyRunRequest(BaseModel):
    ...
    with_narratives:    Optional[bool]      = True
...
async def ask_with_docs(request: HybridQuestionRequest):
    ...
    if request.show_doc_sources and result.get("doc_chunks"):
        doc_sources = list({c["source_file"] for c in result["doc_chunks"]})
...
async def detect_anomalies(request: AnomalyRunRequest):
    ...
    Examples:
      ...
      POST /anomaly/detect {"severity_filter":"CRITICAL"}  → CRITICAL only
...
async def list_anomaly_types():
    ...
    return {"total": len(ANOMALY_TYPE_REGISTRY), "types": ANOMALY_TYPE_REGISTRY}
...
async def run_pipeline(
    ...
    Returns fully enriched signal list sorted by severity and score.
    """
...
async def get_root_cause(_anomaly_id: str, signal: dict):
    """Runs root cause analysis for a provided AnomalySignal dict."""
    s = AnomalySignal(**signal)
    return run_root_cause_analysis(s, with_narrative=True).model_dump()
...
async def get_impact(anomaly_id: str, signal: dict):
    ...
    s = AnomalySignal(**signal)
    return run_impact_analysis(s, with_narrative=True).model_dump()
...
async def get_recommendations(anomaly_id: str, signal: dict):
    ...
    s = AnomalySignal(**signal)
    return run_recommendation_agent(s, with_narrative=True).model_dump()
